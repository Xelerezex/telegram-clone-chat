...
| --------------------- | ----------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Fan-out on Write      | Доставка сообщений и формирование `user_updates`                              | При отправке одного сообщения система сразу создает персональные записи обновлений для всех получателей. Это увеличивает нагрузку на запись, но сильно упрощает и ускоряет чтение `GET /updates`, потому что у каждого пользователя уже есть готовый поток его событий.                                                                                         |
| Keyset Pagination     | Чтение истории чата и чтение обновлений                                       | Вместо `OFFSET` используется курсор, например `seq_no < last_seen_seq_no` или `update_seq_no > last_applied_seq_no`. Такой подход работает быстрее на больших таблицах, потому что база не пропускает большое количество строк, а сразу продолжает чтение от нужной позиции. Для highload-мессенджера это стандартный способ пагинации истории и синхронизации. |
| Delta Synchronization | Синхронизация нескольких устройств через `device_sync_state` и `user_updates` | Устройство хранит последний примененный номер обновления и при следующем запросе получает только изменения после этого курсора. Алгоритм уменьшает сетевой трафик и нагрузку на БД, потому что сервер не пересобирает полное состояние чатов заново, а отдает только дельту после reconnect или offline-периода.                                                |
