...
Для получения картинки использовался сервис [dbdiagram](https://dbdiagram.io/) со кодом из [этого файла](resources/database-sql.md) ([интерактивная ссылка на работу](https://dbdiagram.io/d/highload-logic-scheme-69b28f2477d079431b6ae31a)). 
#### Логическая схема 

![Логическая схема БД](pictures/logic-database.svg)

#### Описание таблиц
| Таблица          | Назначение                                     | Характер данных | Особенности                                                                 |
| ---------------- | ---------------------------------------------- | --------------- | --------------------------------------------------------------------------- |
| `users`          | Профиль пользователя.                          | Постоянные      | Постоянные данные учетной записи.                                           |
| `user_sessions`  | Авторизация и активные устройства.             | Временные, TTL  | Одна учетная запись может иметь несколько активных устройств.               |
| `chats`          | Метаданные диалогов, групп и каналов.          | Постоянные      | `type` различает `private/group/channel is_secret` отмечает секретные чаты. |
| `chat_members`   | Участники чатов и их состояние.                | Постоянные      | Хранит ACL и пользовательское состояние в чате.                             |
| `messages`       | История текстовых сообщений.                   | Постоянные      | Для `secret chats` сервер хранит `ciphertext`, а `is_encrypted = true`.     |
| `update_buffer`  | Буфер обновлений для доставки и синхронизации. | Временные, TTL  | Используется для `getUpdates /reconnect/sync`.                              |
| `presence_cache` | Онлайн-статус и last seen.                     | Временные, TTL  | Нужен для realtime-логики.                                                  |
| `event_log`      | Журнал системных и пользовательских событий.   | Только вставка  | Используется для аудита и аналитики.                                        |
#### Размеры данных и нагрузки на чтение с записью

| Таблица          | Средний размер строки |          Запись, QPS avg/peak |          Чтение, QPS avg/peak | Суточный поток записи, ГБ/сут | Основание                                                                                                            |
| ---------------- | --------------------: | ----------------------------: | ----------------------------: | ----------------------------: | -------------------------------------------------------------------------------------------------------------------- |
| `users`          |                 256 Б |                         1 / 4 |  4,180.56 <br>/ <br>16,722.22 |                         0.022 | Запись профиля редкая. Чтение профиля при старте/восстановлении сессии.                                              |
| `user_sessions`  |                 256 Б |          4,180.56 / 16,722.22 |         19,443.56 / 77,774.26 |                        92.467 | Запись на каждую сессию/reconnect. Чтение на send + sync + session check.                                            |
| `chats`          |                 160 Б |          9,290.79 / 37,163.15 |         15,263.01 / 61,052.04 |                       128.436 | Каждое новое сообщение обновляет `last_message_id` и `updated_at`. Чтение на send + sync.                            |
| `chat_members`   |                  80 Б |        59,722.22 / 238,888.89 | 15,263.01 <br>/ <br>61,052.04 |                       412.800 | Fanout обновляет unread/read-state по каждому получателю. Чтение на ACL + sync state.                                |
| `messages`       |                 640 Б |  9,290.79 <br>/ <br>37,163.15 |   5,972.22<br>/ <br>23,888.89 |                       513.743 | Одна запись на одно отправленное сообщение. Чтение в базовой модели минимально оценивается через sync/history fetch. |
| `update_buffer`  |                 160 Б | 59,722.22 <br>/<br>238,888.89 |        59,722.22 / 238,888.89 |                       825.600 | Одна буферная запись на каждого получателя и одно чтение при выдаче обновления.                                      |
| `presence_cache` |                  64 Б |   4,180.56<br>/ <br>16,722.22 |        59,722.22 / 238,888.89 |                        23.117 | Запись на session/reconnect. Lookup статуса по получателям при fanout.                                               |
| `event_log`      |                 160 Б |        73,193.56 / 292,774.26 |                         0 / 0 |                     1 011.828 | Пишется на send + delivery + session events.                                                                         |

Все расчеты из этой таблицы представлены в [файле](archive/database-calculation.md). 

#### Требования к консистентности
| Таблица                              | Тип консис-тентности | Что должно быть согласовано                                     | Основание                                                                           |
| ------------------------------------ | -------------------- | --------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| `messages` + `chats.last_message_id` | Strong               | Запись нового сообщения и обновление последнего сообщения чата. | Нельзя показывать в чате старое последнее сообщение после успешной отправки нового. |
| `chat_members` при отправке          | Strong               | Проверка членства и роли пользователя в чате.                   | Исключенный пользователь не должен иметь возможность писать.                        |
...
| `messages` в secret chat             | Strong               | Запись ciphertext и флага `is_encrypted`.                       | Сервер не должен смешивать обычные и зашифрованные сообщения.                       |
| `update_buffer` после `messages`     | Eventual             | Появление буферного обновления после фиксации сообщения.        | Сообщение уже durably сохранено, доставка может догонять.                           |
| `presence_cache`                     | Eventual             | online/offline и `last_seen`.                                   | Небольшая задержка статуса допустима.                                               |
| `event_log`                          | Eventual             | Запись события относительно ответа пользователю.                | Журнал можно доливать асинхронно.                                                   |
...
| `event_log`      | `created_at`, `chat_id`, `user_id` | Append-only, естественная ось - время.               | Hot windows по времени, а не по одному ключу.                        |
