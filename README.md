...
| OpenSSL [^40]                                | TLS, сертификаты, криптография                                 | Внешний контур мессенджера должен работать по TLS. OpenSSL нужен для терминации TLS, проверки сертификатов и криптографических примитивов в тех местах, где безопасность обязательна.                                                                           |
...
| OpenTelemetry [^52] | Инструментирование сервиса, метрики, трейсы, логи | В системе много распределённых шагов: API → message service → БД → fan-out → updates. OpenTelemetry нужен как единый стандарт телеметрии, чтобы не городить отдельные несовместимые инструменты под каждый тип сигналов. |
